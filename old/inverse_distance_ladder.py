...
from likelihood_bao import BaseLikelihood, minimize, sample, plot_samples
...
class BAOLikelihood(BaseLikelihood):
    ...
    def set_model(self):
        ...
        self.cosmo_fid = BOSS(engine='class')
        self.hubble_rate_fid = self.cosmo_fid.efunc(self.zeff)
        self.comoving_angular_distance_fid = self.cosmo_fid.comoving_angular_distance(self.zeff)
        self.rs_drag_fid = self.cosmo_fid.rs_drag
    ...
    def model(self, args, z=None, iso=None):
        if z is None:
            ...
            hubble_rate_fid = self.cosmo_fid.efunc(z)
            comoving_angular_distance_fid = self.cosmo_fid.comoving_angular_distance(z)
        params = self.list_to_dict(args)
        rs_drag = params.pop('rs_drag')
        cosmo = Cosmology(**params, engine='class')
        rs_drag *= cosmo['h']
        qpar = hubble_rate_fid * self.rs_drag_fid / cosmo.efunc(z) / rs_drag
        qperp = cosmo.comoving_angular_distance(z) * self.rs_drag_fid / comoving_angular_distance_fid / rs_drag
        toret = []
