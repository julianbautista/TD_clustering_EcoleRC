...
class PowerBAOLikelihood(BaseBAOLikelihood):
    ...
    def set_model(self, cosmo=None):
        self.cosmo = cosmo or get_cosmo_BOSS()
        ...
        if self.iso:
            ...
            self.params['bounds'] = [(0.8,1.2),(0.5,3.0)] + [(-20,20)]*len(bb)
            if not self.recon:
            ...
            self.params['bounds'] = [(0.8,1.2),(0.8,1.2),(0.5,3.0),(0.,2)]
            if not self.recon:
...
def sample(likelihood, nsteps=2000, path_samples=None):
    ...
    if path_samples is not None: np.save(path_samples,samples)
    return samples
...
def list_path_sim_measurement(**kwargs):
    from make_sim_measurements import zeff
...
if __name__ == '__main__':
    ...
    if data == 'sims':
        import make_sim_measurements
        for z in make_sim_measurements.zeff:
            path_data = make_sim_measurements.path_sim_measurement(space=space,tracer=tracer,z=z)
